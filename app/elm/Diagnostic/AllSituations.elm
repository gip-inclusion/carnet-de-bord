...
import CdbGQL.InputObject exposing (buildNotebook_situation_bool_exp, buildTimestamptz_comparison_exp, buildUuid_comparison_exp)
...
import CdbGQL.Object.RefreshNotebookSituationsFromPoleEmploiOutput as Output
...
syncSelector notebookId =
    CdbGQL.Mutation.refresh_notebook_situations_from_pole_emploi
...
notebookIdEq notebookId =
    buildNotebook_situation_bool_exp
        (\args ->
            { args | notebookId = Present (eq notebookId), deletedAt = Present (isnull True) }
        )
...
isnull : Bool -> CdbGQL.InputObject.Timestamptz_comparison_exp
isnull deletedAt =
    buildTimestamptz_comparison_exp
        (\comparison ->
            { comparison | is_null_ = Present deletedAt }
        )
