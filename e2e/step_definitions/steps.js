...
Soit(
	"un {string} authentifié avec l'email {string}",
...
Soit(
	...
	async (email, lastname) => {
		...
		I.amOnPage(
			`/auth/jwt/${uuid}?url=/orientation/carnets/edition/${notebookId}`
		);
	}
);

Soit(
	"le chargé d'orientation {string} sur le carnet de {string}",
	async (email, lastname) => {
		await onBoardingSetup("chargé d'orientation", email, true);
		const uuid = await loginStub("chargé d'orientation", email);
		const notebookId = await goToNotebookForLastName(lastname);
...
Quand(
	"je clique sur la ligne du tableau contenant le texte {string}",
	(text) => {
...
Quand(
	"je renseigne la date {string} dans le champ {string}",
	async (date, input) => {
...
Alors("je vois {string} dans le tableau {string}", (text, tableName) => {
	const locator = locate("tr td").inside(
		locate("//table/caption").withText(tableName)
	);
	I.see(text, locator);
});
...
Alors("je vois {string} dans la tuile {string}", (nb, tileText) => {
	const locator = locate(".fr-card").withDescendant(
		locate("*").withText(tileText)
	);
	I.see(nb, locator);
});
