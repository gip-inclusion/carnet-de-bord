...
function cleanup() {
	status=$?
	echo >&2 "Cleaning up... (status=$status)"
	children=$(jobs -p)
	if [[ -n "$children" ]]; then
		kill $children || true
	fi
...
until curl -s http://localhost:5001/healthz >/dev/null; do
	echo >&2 "-> Hasura is still unavailable - sleeping"
	sleep 1
done

echo >&2 ""
echo >&2 "-> Hasura is up and running on port 5001!"
...
function start_svelte() {
	...
	until curl -s http://localhost:3001/ >/dev/null; do
		echo >&2 "-> Svelte kit is still unavailable - sleeping"
		sleep 1
	done

	echo >&2 ""
	echo >&2 "-> Svelte kit is up and running on port 3001!"
}
...
function start_backend() {
	...
	until curl -s http://localhost:8001/ >/dev/null; do
		echo >&2 "-> Python backend is still unavailable - sleeping"
		sleep 1
	done

	echo >&2 ""
	echo >&2 "-> Python backend is up and running on port 8001!"
}

if [ "$ACTION" = "all" ] || [ "$ACTION" = "js" ]; then
	echo >&2 "-> Starting js tests"
	npm --prefix app test "$@"
fi

if [ "$ACTION" = "all" ] || [ "$ACTION" = "python" ]; then
	echo >&2 "-> Starting Python tests"
