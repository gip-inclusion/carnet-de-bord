CREATE FUNCTION search_notebook_members(search text) RETURNS SETOF notebook_member AS $$
...
WHERE unaccent(search) < % beneficiary.lastname
	OR search < % beneficiary.pe_number
	OR search < % beneficiary.caf_number
	OR search < % beneficiary.mobile_number $$ LANGUAGE sql STABLE;
