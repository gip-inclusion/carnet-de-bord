...
AS $function$
    ...
    WHERE
        ( BS.structure_id = structure_row.id  AND BS.status <> 'outdated' AND (P.structure_id IS NULL OR P.structure_id = structure_row.id))
    OR
        ( NM.active = true  AND P.structure_id = structure_row.id AND BS.structure_id <> structure_row.id)
$function$;
